...
A `GeneSetCollection` is a collection of `GeneSet`s which can be totally unrelated or, as expected, related between them. All the over representative analysis (ORA), such as the hypergeometric test or chi-square test) assume that each gene is independent of other genes. This vignette shows how to check and describe the relationship between the genes and the gene sets to improve existing tools testing over-representation.
...
With `r Biocpkg("GSEABase")` we can see what the collection has but we don't know what is the relationship between genes and gene sets. A first approach would be to use summary to find more information about that relationship:

```{r, summary}
summary(gss)
```
This provide us with some information we already knew, like the number of genes and pathways but also the biggest pathway, and that all genes are in a single gene set.
...
Here genesPerPathways returns the number of genes each pathway have, thus adding them will summarize the total number of genes if they are not duplicated. If they are duplicated, we have the method `nGenes`, which return  `r nGenes(gss)` genes in both pathways.
...
head(ppg)
```
As expected all the genes `all(ppg == 1)` are in a single gene set. If genes would be repeated in several pathways we could use the `nPathways` method which returns what we knew, that there are `r nPathways(gss)` pathways in total.
